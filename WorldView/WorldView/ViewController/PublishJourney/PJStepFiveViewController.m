...
@implementation PJStepFiveViewController
@synthesize pricedescTextView, unitPriceTextFiled, addOnePriceTextFiled, mainService;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setTitle: @"发布旅程"];
    [self loadMainView];
...
- (void)loadMainView
{
    ...
    for(NSInteger i = 0; i < 3; i++)
    {
        ...
        switch (i) {
            case 2:
            {
                [tempLabel setText: @"费用说明:"];
                ///输入框
                pricedescTextView = [[UITextView alloc] initWithFrame: CGRectMake(MARGIN_LEFT, origin_y, tempLabel.frame.size.width, TEXTVIEW_HEIGHT)];
                [pricedescTextView setFont: [UIFont systemFontOfSize: 14.0F]];
                [pricedescTextView setTextColor: [applicationClass methodOfTurnToUIColor: @"#b2b3b4"]];
                [pricedescTextView.layer setBorderColor: [applicationClass methodOfTurnToUIColor: @"#b2b3b4"].CGColor];
                [pricedescTextView.layer setBorderWidth: 0.5f];
                [pricedescTextView.layer setCornerRadius: 3.0f];
                [pricedescTextView setDelegate: self];
                [mainScrollView addSubview: pricedescTextView];
                ///剩余字数
                origin_y += TEXTVIEW_HEIGHT;
                fontNumberLabel = [[XZJ_CustomLabel alloc] initWithFrame: CGRectMake(MARGIN_LEFT, origin_y, pricedescTextView.frame.size.width, 20.0f)];
                [fontNumberLabel setFont: [UIFont systemFontOfSize: 12.0f]];
                [fontNumberLabel setTextAlignment: NSTextAlignmentRight];
                if([[mainService priceDesc] length] > 0){
                    [pricedescTextView setText: [mainService priceDesc]];
                    [fontNumberLabel setText: [NSString stringWithFormat: @"您还可以输入%ld个字", (300 - [[mainService priceDesc] length])]];
                }
                else{
                    [pricedescTextView setText: @"请输入费用说明..."];
            ...
            {
                ///美元符号
                XZJ_CustomLabel *flagLabel = [[XZJ_CustomLabel alloc] initWithFrame: CGRectMake(MARGIN_LEFT, origin_y, TITLE_VALUE_HEIGHT, TITLE_VALUE_HEIGHT)];
                [flagLabel setText:@"$"];
                [flagLabel setTextAlignment: NSTextAlignmentRight];
                [flagLabel setFont: [UIFont boldSystemFontOfSize: 15.0f]];
                [mainScrollView addSubview: flagLabel];
                ///输入框
                origin_x = flagLabel.frame.size.width + flagLabel.frame.origin.x + 10.0f;
                UITextField *textFiled = [[UITextField alloc] initWithFrame: CGRectMake(origin_x, origin_y, (tempLabel.frame.size.width - origin_x) / 2.0f, TITLE_VALUE_HEIGHT)];
                ...
                if(i == 0){
                    unitPriceTextFiled = textFiled;
                    if([[mainService unitPrice] doubleValue] > 0)
                        [unitPriceTextFiled setText: [NSString stringWithFormat: @"%.0f", [[mainService unitPrice]  doubleValue]]];
                    [tempLabel setText: @"单人预约费用:"];
                }
                else{
                    addOnePriceTextFiled = textFiled;
                    if([[mainService addOnePrice] doubleValue] > 0)
                        [addOnePriceTextFiled setText: [NSString stringWithFormat: @"%.0f", [[mainService addOnePrice]  doubleValue]]];
                    [tempLabel setText: @"每增加一个费用:"];
                }
                break;
            }
        }
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    if([[textView text] isEqualToString: @"请输入费用说明..."]){
        [textView setText: @""];
    }
}
