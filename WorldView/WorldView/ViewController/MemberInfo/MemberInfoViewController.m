...
{
    if(success){
        [applicationClass methodOfAlterThenDisAppear: @"更新成功"];
        [self.navigationController popViewControllerAnimated: YES];
    }
    else{
        [applicationClass methodOfAlterThenDisAppear: @"更新失败，请稍候重试"];
    }
}
...
- (void)loadMainView
{
    ...
    ///2.个人资料视图
    CGFloat tableViewHeight = 2 * SECOND_CELL_HEIGHT + 10 * BASE_CELL_HEIGHT + 3 * SECTION_HEIGHT;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if([indexPath section] == 2 && [indexPath row] == 2){
        AboutMeViewController *abloutMeVC = [[AboutMeViewController alloc] init];
        [abloutMeVC setMemberObj: memberObj];
        [self.navigationController pushViewController: abloutMeVC animated: YES];
    }
}
