...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:)])
    {
        ...
        action(delegate, @selector(webImageManager:didFinishWithImage:forURL:), self, image, url);
    }
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didProgressWithPartialImage:forURL:)])
            {
                ...
                //修改版，支持64位
                void (*action)(id, SEL, id, id, id) = (void (*)(id, SEL, id, id, id))objc_msgSend;
                action(delegate, @selector(webImageManager:didProgressWithPartialImage:forURL:), self, image, downloader.url);
            }
            if ([delegate respondsToSelector:@selector(webImageManager:didProgressWithPartialImage:forURL:userInfo:)])
            {
                ...
                //修改版， 支持64位
                void (*action)(id, SEL, id, id, id, id) = (void (*)(id, SEL, id, id, id, id))objc_msgSend;
                action(delegate, @selector(webImageManager:didProgressWithPartialImage:forURL:userInfo:), self, image, downloader.url, userInfo);
            }
        }
    }
}
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
                ...
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:)])
                {
                    ...
                    //修改版，致辞64位
                    void (*action)(id, SEL, id, id, id) = (void (*)(id, SEL, id, id, id))objc_msgSend;
                    action(delegate, @selector(webImageManager:didFinishWithImage:forURL:), self, image, downloader.url);
                }
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)])
                {
                    NSDictionary *userInfo = [[downloadInfo objectAtIndex:uidx] objectForKey:@"userInfo"];
                    ...
                    //修改版，支持64位
                    void (*aciton)(id, SEL, id, id, id, id) = (void (*)(id, SEL, id, id, id, id))objc_msgSend;
                    aciton(delegate, @selector(webImageManager:didFinishWithImage:forURL:userInfo:), self, image, downloader.url, userInfo);
            ...
            else
            {
                ...
                if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:)])
                {
                    ...
                    //修改版，致辞64位
                    void (*action)(id, SEL, id, id, id) = (void (*)(id, SEL, id, id, id))objc_msgSend;
                    action(delegate, @selector(webImageManager:didFailWithError:forURL:), self, nil, downloader.url);
                }
                if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:userInfo:)])
                {
                    ...
                    //修改版，支持64位
                    void (*aciton)(id, SEL, id, id, id, id) = (void (*)(id, SEL, id, id, id, id))objc_msgSend;
                    aciton(delegate, @selector(webImageManager:didFailWithError:forURL:userInfo:), self, nil, downloader.url, userInfo);
                }
#if NS_BLOCKS_AVAILABLE
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:)])
            {
                ...
                //修改版，致辞64位
                void (*action)(id, SEL, id, id, id) = (void (*)(id, SEL, id, id, id))objc_msgSend;
                action(delegate, @selector(webImageManager:didFailWithError:forURL:), self, error, downloader.url);
            }
            if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:userInfo:)])
            {
                NSDictionary *userInfo = [[downloadInfo objectAtIndex:uidx] objectForKey:@"userInfo"];
                ...
                //修改版，支持64位
                void (*aciton)(id, SEL, id, id, id, id) = (void (*)(id, SEL, id, id, id, id))objc_msgSend;
                aciton(delegate, @selector(webImageManager:didFailWithError:forURL:userInfo:), self, error, downloader.url, userInfo);
            }
#if NS_BLOCKS_AVAILABLE
            if ([[downloadInfo objectAtIndex:uidx] objectForKey:@"failure"])
            {
